...
// Fallback: read the TypeScript file and parse slugs
const tsPath = path.join(__dirname, '..', 'src', 'utils', 'blogPosts.ts');
let blogPosts = [];
if (fs.existsSync(tsPath)) {
  const content = fs.readFileSync(tsPath, 'utf8');
  // crude parse: find all slug: "..." occurrences
  const slugRegex = /slug:\s*"([^"]+)"/g;
  let m;
  while ((m = slugRegex.exec(content)) !== null) {
    blogPosts.push({ slug: m[1] });
  }
} else {
  console.warn('Could not find src/utils/blogPosts.ts — sitemap will include only static pages.');
}
